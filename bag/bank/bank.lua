	...
	local AddSlots = function(bank)
		for i = 1, 7 do
			...
			if i == 1 then
				slot:SetPoint('TOPLEFT', bank, 62, -32)
			else
				slot:SetPoint('LEFT', _G['BankFrameBag'..(i - 1)], 'RIGHT', 5, 0)
			end
		end
	end
	...
	local OnEvent = function()
		if  event == 'BANKFRAME_OPENED' then
			SetPortraitToTexture(_G['trustfund'].portrait, 'npc')
		else
			local bank = CreateFrame('Button', 'trustfund', _G['handbag'])
			LADYUI_AddTemplate(bank, true)
			bank:SetPoint('BOTTOMRIGHT', _G['handbag'], 'BOTTOMLEFT', -20, 0)
